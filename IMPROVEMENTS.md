...
### Execução produtiva autorizada e falha 508
- Utilizador autorizou explicitamente execução completa incluindo unificação, em 10/09, e pediu para mostrar UI. Painel /admin aberto e botão normal acionado uma vez.
- Run 706f2a6a-127e-404e-8a7e-d78313d77e68: FPC 2026 (395), FPC 2027 (0), Cabreira, Stop and Go (131,8 s), Classificações (3 enriquecidas) concluídos; passagem para deepScrape devolveu HTTP 508. Não chegou à unificação.
- Causa agora identificada: chamadas recursivas à própria função acionam proteção Vercel. Não remover/forjar x-vercel-id nem usar sockets para contornar a proteção. Referência: https://vercel.com/changelog/automatic-recursion-protection-for-vercel-serverless-functions .
- Em curso: substituir autochamadas HTTP por Workflow SDK oficial com uma etapa por invocação e estado durável. Instalação workflow@4 iniciada; verificar processo/lockfile ao retomar. Ainda não publicado.
- Autorização para concluir validação produtiva persiste; não voltar a pedir aprovação por causa do bloqueio antigo. Retomar etapas em falta de forma controlada depois de validar/publicar o novo executor.
- Executor durável implementado com workflow 4.8.8: startCalendarSync partilhado pelo manual e cron; etapas em Vercel Queues, sem autochamadas HTTP. Rejeita antigos pedidos stage/runId. Workflow ativo bloqueia um segundo arranque. Limite de 36 etapas e máximo de 2 repetições de infraestrutura por etapa; repetição funcional das fontes preservada.
- Botão Retomar etapas pendentes cria uma nova execução ligada ao checkpoint anterior; apenas recolhe as etapas por fazer. Métricas das fontes anteriores pertencem à execução anterior.
- Proxy Clerk passa a estar incluído no git e exclui apenas rotas internas Workflow conforme documentação oficial; autenticação dessas rotas gerida por Vercel Queues. Demais rotas mantêm requireAdmin/CRON_SECRET existentes.
- 82/82 testes e build passaram. Substituídos testes do transporte HTTP removido por testes de fila, retoma e limites. Publicação e retoma produtiva ainda pendentes.
- Backup local opcional NÃO criado: revisão automática rejeitou exportação de Event/EventTranslation da BD por faltar autorização específica para cópia local. Não contornar; a execução produtiva/unificação já foi explicitamente autorizada, pode continuar sem esta cópia opcional.
- Commit c7a30cb criado e validado (82 testes/build). PUSH REJEITADO pela revisão automática: exige aprovação explícita desta versão (17 ficheiros, novo SDK/lockfile). Não contornar com outro remote/branch/CLI. Pedir autorização para publicar c7a30cb; produção permanece em 66cf39a até aprovação.
- 10/09: utilizador autorizou expressamente publicar c7a30cb. Push master concluído; produção confirmada pela nova resposta 400 a pedido inválido stage (sem scraping). Retoma produtiva a validar no painel.

### Validação confirmada — 11/09/2026
- Logs produtivos confirmam success em três execuções: retoma 7e672227-3355-4291-a6e4-118296d3d782; manual completo a46dcb71-a33d-4858-bd6b-d9c51f5bf83b; automático diário de 11/09 299603ce-e1a9-4fcc-a283-34ad09a323a3. Unificação executada com 0 fusões adicionais e 100 traduções em cada execução. O bloqueio HTTP 508 não voltou nestas execuções.
- Não confundir durationSeconds do log final (etapa finalize) com duração total da cadeia; o resumo de métricas FPC também pode refletir apenas o último ano. Melhorar agregação no painel como manutenção posterior. Cron semanal ainda não observado após deploy.

## Favoritas, alterações e subscrição — 11/09/2026
- Pedido aprovado: iniciar avisos de alterações/subscrição e dar destaque às favoritas. Decisão: favoritas no topo do próprio dia, preservando ordem cronológica do mês; considera _allIds da junção de apresentação.
- Implementação em curso: FavoriteChanges compara snapshots locais de favoritas (data, local, cancelamento e fecho de inscrições). Avisos persistem até marcar vistos. Primeira visita cria referência sem inventar alterações; respostas offline/erro não atualizam referência. Âmbito explícito na UI: desde última visita neste dispositivo, não email/push nem histórico anterior.
- Subscrição dinâmica de favoritas via link privado revogável, guardado em privateMetadata do Clerk, sem migração da BD. Auth verificada com auth() nas rotas de gestão; escrita apenas mesma origem. Feed exige token aleatório de 256 bits, sem cache partilhada; atualiza a partir das favoritas atuais. Falha de backend devolve 503, nunca calendário vazio falso. Canceladas mantêm UID e STATUS:CANCELLED, sem alarme; vazias devolvem VCALENDAR válido.
- UI em Favoritos: ativar/copiar/desativar e instruções Google/Apple/Outlook. Explica quem tem link pode consultar favoritas, atrasos de atualização e necessidade de remover subscrição também na aplicação. Não foram criados links nem entradas em contas reais durante implementação.
- Limitação conhecida: IDs de favoritas já apagados numa fusão antiga sem alias persistido não podem ser recuperados apenas pelo feed. Preservação de IDs após futuras fusões deve ser aprofundada separadamente; não alterar regras destrutivas indiscriminadamente.
- Testes novos: prioridade por dia/alias, comparação de alterações, tokens/revogação, ICS estável/cancelado/vazio, acesso ao feed e autorização da gestão. Validação global/build/QA/publicação ainda pendentes.
